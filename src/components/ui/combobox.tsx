  ...
  return (
      <ComboboxPrimitive.Chip
          ...
          className={cn(
              ...
              "flex items-center justify-center gap-1.5 h-7 w-fit rounded-md px-2 text-xs font-medium whitespace-nowrap",
              "transition-colors leading-none",
              className
          )}
          {...props}
      >
        <span className="flex-1 text-center pt-[1px]">{children}</span>

        {showRemove && (
            <ComboboxPrimitive.ChipRemove
                render={<Button variant="ghost" size="icon" className="h-4 w-4 p-0" />}
                data-slot="combobox-chip-remove"
            >
              <XIcon className="size-3 opacity-60 transition-opacity" />
            </ComboboxPrimitive.ChipRemove>
        )}
      </ComboboxPrimitive.Chip>
  )
